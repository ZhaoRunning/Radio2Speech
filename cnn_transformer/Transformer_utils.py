...
class Attention(nn.Module):
    def __init__(self, num_heads, hidden_size, transformer_attention_dropout_rate):
...
class Mlp(nn.Module):
    def __init__(self, hidden_size, mlp_dim, transformer_dropout_rate):
...
class AbsolutePositionalEncoder(nn.Module):
    def __init__(self, emb_dim, max_position=841):
        ...
        self.positional_encoding[0, :, 0::2] = torch.sin(position / (10000 ** (_2i / emb_dim)))
        # PE(pos, 2i+1) = cos(pos/10000^(2i/d_model))
        self.positional_encoding[0, :, 1::2] = torch.cos(position / (10000 ** (_2i / emb_dim)))
        # self.register_buffer('positional_encoding', positional_encoding)

    def forward(self, x):
        # batch_size, input_len, embedding_dim
        batch_size, seq_len, _ = x.size()
        output_pos_embedding = self.positional_encoding[:, :seq_len, :].cuda()

        return output_pos_embedding